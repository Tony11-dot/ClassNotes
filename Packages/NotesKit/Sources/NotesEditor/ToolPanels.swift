...
/// One instrument's settings: a live preview of the stroke, its colour, and the
/// two behaviors that touch handwriting at all — scribble-to-erase and shape
/// snapping. Deliberately nothing else.
///
/// This used to also carry Stability, Tip, Sensitivity, Thickness and
/// Concentration sliders. Stability/Tip/Sensitivity rebuilt every finished
/// stroke's own geometry (`PenShaper`, removed) and were the source of ink
/// coming out shrunk or missing edges no matter how many times that reshaping
/// was patched — the only actually reliable fix was to stop reshaping ordinary
/// handwriting at all. Thickness and Concentration were never part of that
/// problem, but a settings panel that's mostly gone and still has two sliders
/// left in it isn't simpler, it's just smaller — so a pen is now a fixed
/// instrument (see `PenLibrary`), and the only thing left to choose is colour.
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                StrokePreview(color: color, width: settings.effectiveWidth, opacity: settings.concentration)
